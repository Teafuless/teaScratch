        ...
        },
{
        ...
        },
        {
        ...
        },
         {
          ...
          opcode: 'tryLogicBB',
          blockType: Scratch.BlockType.BOOLEAN,
          text: 'попробовать выполнить [CODE]',
          arguments: {
              CODE: {
                  type: Scratch.ArgumentType.STRING,
                  defaultValue: 'tea'
...
return isNaN(Number(DIRECT)) ? 0 : Number(DIRECT)
  ...
  getScreen({PARAM}){
    ...
    } else if (PARAM == 'первый доступный пиксель сверху'){
  ...
  addCodeLine({NUM,CODE,NAME}){
    if (NAME in window === false){
      globalThis[NAME] = {}
    }
    let res = `globalThis['${NAME}'].line${NUM} = 'String(${CODE})'`
    eval(res)
  }
    ...
    let res = `globalThis['${NAME}'].line${NUM}`
    return eval(res).replaceAll('/*new line*/','\n')
  }
        executeCode({NUM,NAME}){
let res = ''
for (let i in globalThis[NAME]){
...
for (let i in globalThis[NAME]){
  ...
  return res.replaceAll('/*new line*/','\n')
}
