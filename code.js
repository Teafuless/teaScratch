...
return String(date.getUTCHours())
  ...
  mixLetters({TEXT,REPEAT}){
    ...
    Math.floor(Math.random() * (max - min + 1)) + min;
      ...
      for (let i = 0;i<TEXT.length;i++){
        res += TEXT.charAt(random(0,TEXT.length))
      }
    } else {
      let unusedText = TEXT
      let num;
    ...
    Math.floor(Math.random() * (max - min + 1)) + min;
            ...
            for (let i = 0;i<TEXT.length;i++){
              num = random(0,unusedText.length)
...
return String(res)
  ...
  mixWords({TEXT,REPEAT}){
    ...
    if (REPEAT=='true'||REPEAT==true){
      for (let i = 0;i<text.length;i++){
        res += String(text[random(0,text.length-1)])
      }
    } else {
      let unusedText = text
      let num
      for (let i = 0;i<text.length;i++){
        num = random(0,text.length-1)
        res += String(text[num])+' '
        delete unusedText[Object.keys(unusedText)[i]]
      }
    }
    return String(res)
  }
